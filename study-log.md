...
- 按学习启动协议先同步仓库并读取 `HANDOFF.md`、`conversation-log.md`、`study-log.md` 和 `java-backend-interview-plan.md`。
- 用户要求用小白能理解的方式讲 HashMap，减少术语密度。
- 已用“柜子和格子”的比喻讲清：
  - HashMap 为什么要根据 key 算位置：避免从头遍历，提升查询效率。
  - 为什么希望元素分散：减少哈希冲突，避免一个格子里继续找很多次。
  - 为什么容量是 2 的幂：便于用 `(n - 1) & hash` 快速计算位置，并尽量让元素分散。
  - 为什么扩容通常扩大 2 倍：继续保持容量是 2 的幂，搬迁元素时位置变化更简单。
- 用户已能回答：
  - HashMap 可以通过 key 快速定位 value。
  - 哈希冲突是多个 key 算到同一位置。
  - 容量是 2 的幂和 `(n - 1) & hash` 有关。
  - 超过阈值时触发扩容。
  - HashMap 的插入、覆盖等操作不是原子的，多线程下可能出现覆盖和数量错误。
- 当前卡壳点：
  - `put` 流程还容易答得过短，需要补上“算 hash -> 算下标 -> key 相同覆盖 -> key 不同挂链表/红黑树”。
  - 哈希冲突处理不能说“排序”，应说“链表或红黑树”。
  - 还未完成第二轮追问：链表转红黑树、equals/hashCode、线程不安全扩展说明。

### 下一步

- 继续 HashMap 第二轮 5 题：
  1. HashMap 解决哈希冲突的方式是什么？
  2. 链表什么时候会转成红黑树？
  3. 为什么链表太长以后要转成红黑树？
  4. HashMap 的 key 为什么要重写 `equals` 和 `hashCode`？
  5. 如果两个对象 `equals` 相等，但 `hashCode` 不一样，放进 HashMap 会有什么问题？
